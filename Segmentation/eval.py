...
class Trainer(object):
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            loss_distill = loss_distill.mean() / 70.0 * 1000.
            loss = loss_seg + loss_distill
            loss.backward()
            optimizer.step()
            train_loss += loss.item()
            total_seg_loss += loss_seg.item()
            total_dis_loss += loss_distill.item()
            tbar.set_description('Train loss: %.3f, cur loss seg: %.3f, cur loss dis: %.3f' % (train_loss / (i + 1), (total_seg_loss / (i + 1)), (total_dis_loss / (i + 1))))
...
def main():
    ...
    trainer.s_net.module.load_state_dict(ckpt['state_dict'])
