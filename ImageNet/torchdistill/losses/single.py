...
@register_single_loss
class ChSimLossKernel(nn.Module):
    """
    """
    def __init__(self, feature_pairs, **kwargs):
        super().__init__()
    ...
    def compute_cc_mat_by_gaussian_rbf(self, linear_outputs):
        bsz, ch = linear_outputs.shape[0], linear_outputs.shape[1]
        row_list = list()
        for index in range(ch):
            row = 1
            right_term = torch.bmm(linear_outputs, linear_outputs[:,index,:].unsqueeze(2))
            for p in range(1, self.max_p + 1):
                left_term = ((2 * self.gamma) ** p) / (math.factorial(p))
                row += left_term * (right_term ** p)
            row *= math.exp(-2 * self.gamma)
            row_list.append(row.squeeze(0))
        return torch.stack(row_list, dim = 2)
    ...
    def batch_loss(self, f_s, f_t):
        ...
        teacher_cc = self.compute_cc_mat_by_gaussian_rbf(f_s)
        student_cc = self.compute_cc_mat_by_gaussian_rbf(f_t)
        cc_loss = torch.dist(student_cc, teacher_cc, 2) / (ch*bsz*bsz)
        return cc_loss
